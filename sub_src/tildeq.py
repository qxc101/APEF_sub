...
# Self-contained copy of `domain_metric/src/tildeq.py` (torch-based).
...
def _require_torch() -> None:
    if torch is None:  # pragma: no cover
        raise RuntimeError(
            "torch is required for TILDE-Q but is not available in this environment."
...
def phase_loss(outputs, targets):
    ...
    phase_loss_val = (torch.mean(zero_error) + torch.mean(ae)) / (T**0.5)
