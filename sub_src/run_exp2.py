...
if __package__ in {None, ""}:  # allow `python sub_src/run_exp2.py ...`
    import sys
...
from sub_src.annotations import read_pairwise_annotations_csv, win_loss_scores
...
from sub_src.paths import data_dir, ensure_results_dir
...
def run_exp2(
    *,
    ...
    if dataset in {"co2", "gpp"}:
        ann_path = data_dir() / "exp2_annotations" / "final_annotations.csv"
        winner_col = "CO2 Winner_Final" if dataset == "co2" else "GPP Winner_Final"
        m1_cols = ["Model 1_A1", "Model 1"]
        m2_cols = ["Model 2_A1", "Model 2"]
    elif dataset == "camels":
        ann_path = data_dir() / "exp2_camels_methane_annotations" / "CAMELS_final_annotations.csv"
        winner_col = "Overall Winner"
        m1_cols = ["Model 1", "Model 1_A1"]
        m2_cols = ["Model 2", "Model 2_A1"]
    elif dataset == "xmethanewet":
        ann_path = data_dir() / "exp2_camels_methane_annotations" / "XMethaneWet_final_annotations.csv"
        winner_col = "Overall Winner"
        m1_cols = ["Model 1", "Model 1_A1"]
        m2_cols = ["Model 2", "Model 2_A1"]
    else:
    ...
    comps = read_pairwise_annotations_csv(
        ann_path, winner_column=winner_col, model1_columns=m1_cols, model2_columns=m2_cols
    )
    winloss = win_loss_scores(comps, n_models=20)
    target_scores = [float(v) for v in winloss]
    ...
    covered = sorted({c.ts_a for c in comps} | {c.ts_b for c in comps})
    write_json(
        result.run_dir / "annotations.json",
        {
            "path": str(ann_path),
            "winner_column": winner_col,
            "n_comparisons": len(comps),
            "covered_ts_ids": covered,
        },
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run exp2 (human pairwise preferences) in sub_src.")
